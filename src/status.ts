/**
 * Check Secretless protection status for a project.
 */
...
export function status(projectDir: string): StatusResult {
  ...
  // Check which tools have Secretless instructions
  ...
  for (const tool of detected) {
    ...
    if (fs.existsSync(filePath)) {
      try {
        ...
        if (content.includes('secretless:managed') || content.includes('Secretless')) {
